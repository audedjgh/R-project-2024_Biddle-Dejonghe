...
clean_dataset <- function(df) {
  id_columns <- c("Country", "country", "COUNTRY", "Year", "year", "YEAR")
...
suppressMessages({
  Better_Life_Index_2022 <- read_delim(
    file = "Clean 2022 Better Life Index.csv", 
    delim = ";", 
    escape_double = FALSE, 
    trim_ws = TRUE,
    na = na_values,
  ...
  GDP <- read_excel(
    path = "GDP (current US$).xls", 
    sheet = "Data",
    na = na_values
  ) %>% clean_dataset()
  
  Gas_emissions <- read_excel(
    path = "GovReg_GHG proxy_2023.xlsx", 
  ...
  Share_of_renewable_sources <- read_excel(
    path = "Share of energy from renewable sources.xlsx", 
    sheet = "EU27",
    na = na_values
  ) %>% clean_dataset()
})
...
Our analysis begins with a chart representing the average score for the student skills and life satisfaction categories each country obtained in 2022...
