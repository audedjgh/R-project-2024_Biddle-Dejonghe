...
# Step 1: Filter Education dataset
Education_filtered <- Education |>
  ...
  select(-c(`2000`, `2001`, `2002`, `2003`, `2023`)) # Remove years 2000, 2001, 2002, 2003, and 2023

# Step 2: Filter Renewable Energy dataset (years 2004-2022)
RenewableEnergy_filtered <- Overall_share_of_renewable_sources |>
  select(`Country Name`, `2004`:`2022`) # Filter only years 2004-2022
...
combined_data <- combined_data |>
  rename_at(vars(starts_with("2004")), ~ paste0("RenewableShare_", .))
...
# Step 6: Create categories for education levels based on mean and standard deviation thresholds

# Extract the relevant columns for education data (2005-2022)
relevant_columns <- combined_data[, c("RenewableShare_2004.x", "2005.x", "2006.x", "2007.x", "2008.x", "2009.x", 
                                      "2010.x", "2011.x", "2012.x", "2013.x", "2014.x", 
                                      "2015.x", "2016.x", "2017.x", "2018.x", "2019.x", 
...
# Categorize each education value based on the thresholds
long_data <- long_data |>
  mutate(EducationLevel = case_when(
    `Value` >= high_thresholds[as.integer(substr(Year, 1, 4)) - 2004 + 1] ~ "High", # Check if value is above the high threshold
    `Value` <= low_thresholds[as.integer(substr(Year, 1, 4)) - 2004 + 1] ~ "Low",  # Check if value is below the low threshold
    TRUE ~ "Medium"  # Otherwise, categorize as Medium
  ))

# Step 7: Create the boxplot
ggplot(long_data, aes(x = EducationLevel, y = RenewableShare, fill = EducationLevel)) +
  geom_boxplot() +
  labs(
